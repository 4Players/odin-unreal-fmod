...
class ODIN_API UOdinAudioCapture : public UAudioCapture, public FTickableGameObject
{
  ...
  public:
    ...
    int OnGeneratedAudio2(const float* AudioData, int32 NumSamples)
    {
        OnGeneratedAudio(AudioData, NumSamples);
        if (AudioData[0] > 0.01)
        {
            UE_LOG(LogTemp, Warning, TEXT("%f,%f,%f,%f,%f,%f"), AudioData[0], AudioData[1], AudioData[2], AudioData[3], AudioData[4], AudioData[5]);
        }
        return 0;
    };
